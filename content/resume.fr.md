+++
title = ''
hideMeta = true
+++

# Pierre Giusti ッ {style="text-align:center"}
 > Après 10 ans d'expérience en développement, je suis aujourd'hui focalisé sur le pentest et la cybersécurité offensive. J’aime creuser les sujets techniques, apprendre et développer des outils.  Parcours atypique : charpentier, entrepreneur, traversée de l’Atlantique à la voile. Quand je m'engage dans un projet, je vais au bout.

#### France\/Pyrénées <br> [ [piergst@protonmail.com](mailto:piergst@protonmail.com) ] . [ +33 6 77 23 86 65 ] <br> [ [blog](https://piergst.github.io) ] . [ [github](https://github.com/piergst) ] . [ [linkedin](https://www.linkedin.com/in/pierre-giusti-8aaa1b2b6)] . [ [hackthebox](https://app.hackthebox.com/public/users/910360) ]{style="text-align:center; line-height:1.9"}

## Expériences professionnelles {.resume-h2}

### Sécurité offensive & développement IA @Allvei <date>Janvier 2026 - En cours / Full Remote</date> {.resume-h3}

Startup spécialisée dans l'automatisation du pentest par IA.

**R&D - Plateforme de recherche de vulnérabilités automatisée par IA :** {{< resume/tag "Python" >}} {{< resume/tag "Agno" >}}

- Recherche manuelle de vulnérabilités (bug bounty, labs AD, labs web), reproduction des chemins d'exploitation via la plateforme, et itération jusqu'à détection automatisée
- Développement d'un framework d'évaluation pour agents LLM de pentest : scorers multi-niveaux (embeddings, LLM-as-judge), et détection de régression
- Agent de reconnaissance web autonome avec dashboard temps réel {{< resume/tag "FastAPI" >}} {{< resume/tag "bbot" >}}
- Pipeline de collecte et d'enrichissement automatisé d'une base de connaissances cybersécurité (crawling, classification et scoring par LLM)

**Développement d'outils et infrastructure de test :** {{< resume/tag "Python" >}} {{< resume/tag "Bash" >}} {{< resume/tag "Docker" >}}

- Toolset de reconnaissance modulaire pour la cartographie de surface d'attaque
- Setup et maintien d'infrastructures de lab {{< resume/tag "Proxmox" >}} {{< resume/tag "Ludus" >}}
- Documentation technique et veille cybersécurité

### Développeur @Sofia Development <date>2022 - Septembre 2025 / Full remote</date> {.resume-h3}

Solutions logicielles pour le domaine de la santé, à destination des praticiens libéraux (infirmiers, kinésithérapeutes...).

**Développement**

* Instrumentation du backend d'applications web et optimisation des performances {{< resume/tag "C#/.NET/LINQ" >}} {{< resume/tag "SQL" >}} {{< resume/tag "Azure AppInsight" >}}
* Développement d'outillage pour la migration de bases clients {{< resume/tag "C#/.NET" >}} {{< resume/tag "SQL" >}} {{< resume/tag "Windows" >}}
* Développement mobile multiplateforme {{< resume/tag "C++/Qt/QML" >}} {{< resume/tag "iOS" >}} {{< resume/tag "Android" >}}
* Développement client lourd multiplateforme {{< resume/tag "C++/Qt" >}} {{< resume/tag "macOS" >}} {{< resume/tag "Windows" >}}
* Développement backend {{< resume/tag "C++/Qt" >}} {{< resume/tag "Linux" >}} {{< resume/tag "Node.js" >}}
* Mise en place de tests unitaires et tests d'intégration
* Rédaction de documentation

**Devops** {{< resume/tag "Azure" >}} {{< resume/tag "Docker" >}}

* Mise en place et supervision CI/CD
* Mise en place et supervision de services Cloud
* Mise en place et supervision des environnements de développement

### Cofondateur et CTO @Epur <date>2015 - 2022</date> {.resume-h3}

Création de l’entreprise, du concept initial jusqu’à la production et la vente de 100 unités par an (environ 2,5 M€ de chiffre d'affaires annuel et 25 salariés). Développement et commercialisation d'une machine numérique mobile pour la fabrication de charpente bois.

**Prototypage**

* Conception, développement et fabrication des deux premiers prototypes, intégralement réalisés par mes soins.

**Développement** {{< resume/tag "C++/Qt/QML" >}} {{< resume/tag "Assembleur" >}} {{< resume/tag "Linux Embarqué" >}}

* Constitution et direction technique d’une équipe de développement en systèmes embarqués
* Développement d'applications graphiques embarquées et de software bas niveau pour le pilotage hardware.
* Développement d'algorithmes spécifiques de génération de parcours d’usinage.

**R&D & Industrialisation**

* Pilotage de la recherche et développement en mécanique, électronique et software.
* Industrialisation des solutions techniques

**Fondateur**

* Gestion opérationnelle de l'entreprise
* Gestion de projet
* Management d'équipe
* Supervision de la chaîne d'approvisionnement.
* Levée de fonds pour le financement de la production.

### Charpentier <date>2005 - 2015</date> {.resume-h3}

- Indépendant spécialisé en construction de maisons à ossature bois et charpentes traditionnelles (2011 - 2015).
- Fabrication de voiliers classiques en bois (2010).
- Construction de maisons en rondins, avec une expérience au Canada (2005 - 2010).

## Veille & pratique personnelle {.resume-h2}

### Sécurité offensive {.resume-h3}

- Bug bounty (bounties en cours chez **Paypal**)
- Web security sur PortSwigger Academy (+65% des labs)
- Web, Active Directory, Linux, Machines & Prolabs (Dante 100%) sur HackTheBox (Hacker / Professional Rank)

### Formation Cybersécurité @Jedha <date>Octobre - Décembre 2025</date> {.resume-h3}

- Formation intensive pour le TP Administrateur d'infrastructures sécurisées
- Sécurité des applications, réseaux, systèmes et Cloud
- Mise en pratique sur labs, exercices d'attaque/défense

## Langages & environnements {.resume-h2}

* **Langages de programmation :**
  * Avancé : Python, Bash, C++/Qt, QML, C#/.NET
  * Intermédiaire : JS, HTML, CSS, SQL, PowerShell
  * Notions de base : Rust
* **Systèmes :** Linux (Debian, Arch), macOS, Windows
* **Cloud & Devops :**
  * Azure, Docker, Podman, Proxmox
  * Notions de base : AWS, Docker swarm, Ansible, Terraform
* **Environnements & IDE :** GitHub, GitLab, VSCode, Neovim, QtCreator, Visual Studio
* **IA & LLM :** Développement IA, flows agentiques, évaluation d'agents LLM
* **Offensif :** Exegol, Burp Suite, Caido
* **Langues :** Français (natif), Anglais (proficient)


## Diplômes {.resume-h2}

- **TP Administrateur d'infrastructures sécurisées** (En cours)
- **BTS Système numérique** (2017)
- **Diplôme de charpentier naval** (2011)
- **Diplôme de charpentier bois** (2010)
- **Baccalauréat scientifique** (2004)

##### [ :) ] {style="text-align:center"}

{{< resume/tag "faire beaucoup avec peu" >}} {{< resume/tag "l'open source" >}} {{< resume/tag "le travail bien fait" >}}
{{< resume/tag "la montagne" >}} {{< resume/tag "la simplicité" >}} {{< resume/tag "linux" >}} {{< resume/tag "le travail du bois" >}} {{< resume/tag "l'océan" >}} {{< resume/tag "les bons repas" >}}
